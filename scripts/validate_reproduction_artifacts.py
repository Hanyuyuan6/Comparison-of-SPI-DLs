"""Validate the exact Table-I reproduction matrix and write a hash manifest."""
...
def validate(
    ...
    for label, dataset, bucket_size, expected_n in CONDITIONS:
        path = root / "table1" / f"physics_{label}_{bucket_size}.json"
        expected_paths.add(path.resolve())
        raw, payload = _read_json(path)
        expected = {
            "schema_version": 1,
