#!/usr/bin/env bash
# Reproduce the released reconstruction-probe benchmark from committed configs.
...
mkdir "$RESULT_ROOT/profile" "$RESULT_ROOT/matrix" "$RESULT_ROOT/baselines"
...
# 0) Prepare exactly the benchmark datasets. torchvision owns the trusted
...
# 3) Hadamard-adjoint physics row: all five benchmark conditions.
for bucket_size in 512 1024 2048; do
  run_gi_eval \
    mnist "$bucket_size" \
    "$RESULT_ROOT/matrix/physics_mnist_${bucket_size}.json" \
    "$RESULT_ROOT/baselines/gi_mnist_${bucket_size}"
done
run_gi_eval \
  fashion_mnist 512 \
  "$RESULT_ROOT/matrix/physics_fmnist_512.json" \
  "$RESULT_ROOT/baselines/gi_fmnist_512"
run_gi_eval \
  celeba 512 \
  "$RESULT_ROOT/matrix/physics_celeba_512.json" \
  "$RESULT_ROOT/baselines/gi_celeba_512"

# Optional CS diagnostic (not a benchmark-matrix row and prohibitively slow at full scale).
