...
# Exact parameter counts at the paper setting (bucket_size=512, img_size=128),
# pinning the Params column of Table I. Comment shows the paper's rounded M.
EXPECTED_PARAMS = {
    ...
    def test_all_models_consistency(self, test_config):
        ...
        for model_name, extra_params in model_configs.items():
            try:
                ...
                # verify parameter count matches the paper's reported value
                ...
                assert num_params == EXPECTED_PARAMS[model_name], (
                    ...
                    f"{EXPECTED_PARAMS[model_name]:,} (paper Table I). A config or "
                    f"architecture change has altered a reported number.")
