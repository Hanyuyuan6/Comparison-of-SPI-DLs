...
def test_reproduction_validator_requires_exact_matrix_and_identity(tmp_path):
    ...
    for key in MODEL_KEYS:
        for label, dataset, bucket_size, n in CONDITIONS:
            ...
            (root / 'matrix' / f'{name}.json').write_text(
                json.dumps(payload), encoding='utf-8')
    for label, dataset, bucket_size, n in CONDITIONS:
        ...
        (root / 'matrix' / f'physics_{label}_{bucket_size}.json').write_text(
            json.dumps(payload), encoding='utf-8')
    ...
    lineage = root / 'matrix' / 'fn_mnist_512_seed42_r1.json'
    ...
    wrong = root / 'matrix' / 'gru_mnist_512_seed42_r1.json'
